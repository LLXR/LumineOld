from Lumine.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Lumine import SUDO_USERS
...
async def Lumine_is_admin(chat_id: int):
    status = False
    Lumine = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Lumine.id == user.id:
            status = True
            break
    return status
